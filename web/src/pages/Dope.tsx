            ...
            {cards.map((c) => (
              <li key={c.id} className="flex items-center justify-between rounded-lg bg-[var(--panel-2)] px-3 py-2">
                ...
                <div className="flex shrink-0 gap-2">
                  ...
                  <button onClick={() => delCard(c.id)} aria-label={`Apagar cartão ${c.name}`} className="rounded-md border border-[var(--border)] px-2 py-1 text-xs text-red-400">×</button>
                </div>
              </li>
            ))}
          </ul>
          ...
          {loads.map((l, i) => (
            <div key={i} className="grid grid-cols-[1fr_auto] items-end gap-2">
              ...
              <button
                ...
                aria-label={`Remover ${l.name || "carga"}`}
